...
public class TwoPersonGuardTest {
    ...
    void accept_user_and_friend_ok() {
        var cfg = GuardConfig.of("홍길동", "홍길동2");
        var guard = new TwoPersonGuard(cfg);

        assertDoesNotThrow(() -> guard.acceptSpeaker("홍길동"));
        assertDoesNotThrow(() -> guard.acceptSpeaker("홍길동2"));
    }
    ...
    void reject_third_speaker() {
        ...
        assertDoesNotThrow(() -> guard.acceptSpeaker("홍길동"));
        assertDoesNotThrow(() -> guard.acceptSpeaker("홍길동2"));
        IllegalArgumentException ex = assertThrows(IllegalArgumentException.class, () -> guard.acceptSpeaker("홍길동3"));
        assertTrue(ex.getMessage().contains("홍길동3"));
    }
    ...
    void mapToUF_should_map_user_to_U_and_friend_to_F() {
        ...
        assertEquals(Speaker.F, guard.mapToUF("홍길동2"));
        assertNull(guard.mapToUF("홍길동3")); // 제3자는 null 반환
    }
}
