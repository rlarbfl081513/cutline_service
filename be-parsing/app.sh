...
# Spring Boot Application Docker Compose 관리 스크립트
...
case "$1" in
    start)
        ...
        echo "📍 애플리케이션: http://localhost:8081/api/v1"
    ...
    stop)
        ...
        ;;
    restart)
        echo "🔄 Spring Boot 애플리케이션을 재시작합니다..."
        docker-compose -f $COMPOSE_FILE down
        docker-compose -f $COMPOSE_FILE up -d
    ...
    health)
        ...
        curl -f http://localhost:8081/api/v1/actuator/health || echo "❌ 헬스체크 실패"
        ;;
    *)
        echo "CutLine Spring Boot 애플리케이션 관리 스크립트"
        echo ""
